...
func reduceLocalEnumField2[S any](editStruct *S, enumField reflect.Value, index int, fromStruct reflect.Value, f *Field) {
	...
	zalert.PresentOKCanceledView(fview, title, att, wildCards, func(ok bool) (close bool) {
		if ok {
			...
			ForEachField(editStruct, params.FieldParameters, nil, func(each FieldInfo) bool {
				...
				if !each.ReflectValue.IsZero() && sliceLength > 1 && each.ReflectValue.Kind() == reflect.String {
