...
func (v *HorEventsView) updateBlockView(blockIndex int, isNew bool) {
	...
	go v.GetEventViewsFunc(blockIndex, isNew, func(childView zview.View, x int, cellBox zgeo.Size, laneID, rowType int64, blockDone bool) bool {
		if blockDone {
			// if isNew {
		...
		bView, _ := v.horInfinite.FindViewForBlockIndex(blockIndex)
		if bView != view {
...
func (v *HorEventsView) SetLanes(lanes []Lane) {
	...
	for i, lane := range lanes {
		...
		for j, r := range lane.Rows {
			// zlog.Info("SetLaneRow:", y, lane.Name, r.Name, r.ID, r.Height)
			r.y = y
			v.lanes[i].Rows[j] = r
			y += r.Height
		}
...
func (v *HorEventsView) createLanes() {
	...
	for i, lane := range v.lanes {
		...
		for j, r := range lane.Rows {
			...
			v.horInfinite.VertOverlay.Add(rowTitle, zgeo.TopLeft, zgeo.SizeD(v.GutterWidth+2, r.y+laneTitleHeight+v.timeAxisHeight)).Free = true
			y = r.y + r.Height
