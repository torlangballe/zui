...
const emailKey = "zview.AuthenticationEmail"
...
func doAuth(view zview.View, a zusers.Authentication, got func(auth zusers.AuthenticationResult)) {
	...
	if err != nil {
		zalert.ShowError(err, "Authenticate Call Error")
		return
	}
...
func CheckAndDoAuthentication(client *zrpc.Client, canCancel bool, got func(auth zusers.AuthenticationResult)) {
	const tokenKey = "zview.AuthenticationToken"
	...
	if zrpc.ToServerClient.ID != "" {
		...
		zalert.ShowError(err, "Authentication Error")
	}
